...
2026-06-25 with `repetitions = 1`, `live_fresh_scan_cells = false`, and low
scenario caps. It includes capped real Mode 1 S0/S1/S4/S5 evidence, capped
fresh-funded Mode 2 S0/S1/S4/S5 evidence from `.bench-data/new-wallet-fresh-proof`,
and capped real Mode 3 PP S0/S1/S4/S5 evidence with PP scan cells marked
...
repetition in place and keeping top-level rows confirmed-only.

The promoted V5 proof avoids the polluted original Mode 2 DB. The fresh-proof
wallet matches `HARNESS_SEED_NEW_FRESH` and was funded from the old wallet with
six independent `0.09 T` one-sided outputs mined at height `711302`. A supported
scanner catch-up to height `711305` made those outputs spendable. The promoted
profile then verified Mode 2 S1 tx `3342988131844877001` at height `711336`, S4
tx `5756695120974193262` at height `711341`, and S5 tx
`5402626848094413870` at height `711345` through
`base_node_transaction_query`. The wallet DB still reported those rows as
`broadcast`, so the public base-node query is the confirmation source.

Mode 3 remains deliberately conservative. The promoted run accepted PP S1/S4/S5
batches, but the profile snapshot found them `PENDING_BATCHING`, so those cells
are failed observations with `verification_source=payment_processor_db_observed`
and no top-level confirmed rows. A later DB inspection may find terminal PP
