...
- Mode-local execution is strictly chronological:
  `B0, S0, S1, S2, S3, S4, S5, S6, S7`. Every scan repetition wipes its data
...
- Mode 2 uses settlement gates between S1 rounds and between S4 and S5. The
  ...
  than retrying failed sends. It follows a moving public tip and performs a
  second state refresh after independent `C_min` proof before checking the exact
  S1 output/balance invariant.
...
- After PP chain confirmation, the harness waits for the companion receiver DB
  to observe the exact expected output count and balance before evaluating S1.
  This is state convergence, not a transaction retry.
...
The committed `baselines/esmeralda_baseline.json` is historical evidence only.
It is schema v3, was generated on `2026-07-02T22:16:39.401016Z`, and fails the
current schema-v4/submission contract. It must not be described or published as
the current submission baseline. A replacement must come from a fresh namespace
and pass `validate-profile --submission` without manual profile repair.
