...
- Mode 1 uses a real `minotari_console_wallet` process with gRPC. The harness
  manages lifecycle and records submit-time SQLite `database is locked` retries as
  metrics instead of hiding them.
...
- `fund-one-sided` is also outside the measured benchmark path. It reuses the
  ...
  seeds from recovered minotari signing wallets.
